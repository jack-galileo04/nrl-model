...
player_forms_df <- read_csv(here("Data/player_feature_engineered_df.csv"), col_select = -1)
team_recovery_elo_df <- read_csv(here("Data/team_feature_engineered_df.csv"), col_select = -1)
...
xgb_tune_res |> collect_metrics() |> 
  ...
  geom_errorbar(aes(ymin = mean - std_err, ymax = mean + std_err), colour = "red", width = 1) +
  geom_point(size = 2) +
