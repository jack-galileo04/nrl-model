...
setwd("C:/Users/JackG/OneDrive/Documents/Projects/Modelling/nrl-model")

# stopifnot(exists("get_nrl_odds"))
...
lineup_url <- paste0("https://www.nrl.com/news/", lineup_tuesday, "/nrl-team-lists-round-", round_number, "/")
...
First, collect data:
- Get original fetch data
- Update files with previous round data

Get prediction features:
- Fetch upcoming round data (lineups, ladder, team)
- Run through feature engineering pipeline (with historical data) to get 'model_data'.

Modelling:
- Split model data into train (all past data), test (new round data)
- Fit saved model onto train data, predict on test data
...
ladder_df <- read_csv(here("Data/Raw/Ladder Data.csv")) |> 
  group_by(season, round, team) |> 
  slice_head(n = 1) |> # Removes any duplicate records
  ungroup()

player_df <- read_csv(here("Data/Raw/Player Data.csv")) |> 
  group_by(season, round, player_id) |> 
  slice_head(n = 1) |> # Removes any duplicate records
  ungroup()

team_df <- read_csv(here("Data/Raw/Team Data.csv")) |> 
...
prev_ladder_df <- fetch_ladder_nrl(season = season, round_number = round_number-1) |> 
      select(team, ladder_points = points, points_for, points_against, points_diff) |> 
      mutate(season = season, round = round_number-1, ladder_position = row_number()) |> 
...
current_ladder_df <- fetch_ladder_nrl(season = season, round_number = round_number) |> 
      select(team, ladder_points = points, points_for, points_against, points_diff) |> 
      mutate(season = season, round = round_number, ladder_position = row_number()) |> 
  mutate(team = ifelse(team == "Wests Tigers", "Tigers", team),
         team = ifelse(team == "Sea Eagles", "Eagles", team))
```
...
team_recovery_elo_df <- team_and_recovery_df |> 
  ...
  left_join(elo_df |> 
              select(2:10) |> 
              rename(team_name = team), by = c("match_id", "team_name")) |> 
  mutate(result = score) |> 
  arrange(utc_start) |> 
  group_by(team_name) |> 
  mutate(across(
...
model_data <- player_forms_df |> 
  ...
  ungroup() |> 
  left_join(team_recovery_elo_df |> 
              select(-utc_start, -team_location, -competition_id, -round), 
            by = c("match_id", "team_name")) |> 
  select(result, everything()) |> 
  pivot_wider(
    id_cols = c(match_id, date, round),
    names_from = team_location,
    values_from = c(result, team_name, m_per_run:longelo),
    names_sep = "_"
  ) |> 
  unnest() |> # fix this later
  group_by(match_id) |> 
  slice_head(n = 1) |> 
  ungroup() |> 
  mutate(
  ...
  mutate(season_stage = factor(case_when(
    round < 12 ~ "Early",
...
```{r}#
# safe pull of odds data

cache_file <- here("Outputs", paste0("Odds_api_", Sys.Date(), ".csv"))

if (file.exists(cache_file)) {
  odds_raw <- readr::read_csv(cache_file, show_col_types = FALSE) |> 
  ...
  error = function(e){
    message("Odds pull skipped: ", conditionMessage(e))
    tibble(
      match_id = character(), commence_time = as.Date(character()), home_team = character(), away_team = character(), bookmaker = character(), 
      market = character(), last_update = as.Date(character()), home_team_odds = character(), away_team_odds = character()
    )
  }
)
  
  readr::write_csv(odds_raw, cache_file)
}
```
...
```{r}
# combining odds data with prediction
...
bets <- predict(final_fit, new_data = pred_data, type = "prob") |> 
  ...
  group_by(match_id) |> 
  mutate(best_home_odds = max(home_team_odds),
  ...
  arrange(odd_diff) |> 
  slice_head(n = 1) |> 
  select(match_id, date, home_team, away_team, home_team_odds, away_team_odds, bookmaker, ev_bet) |> 
  ungroup()
