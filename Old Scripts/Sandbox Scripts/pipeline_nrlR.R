...
# In case of special round 
lineup_url <-  paste0("https://www.nrl.com/news/", lineup_tuesday, "/nrl-team-lists-magic-round/")
...
ladder_df <- read_csv(here("Data/Raw/Ladder Data.csv"), col_select = -1) |> 
  ...
  filter(!(season == season & round > (round_number-2) )) |> # Gets rid of future rounds
  group_by(season, round, team) |> 
  slice_head(n = 1) |> # Removes any duplicate records
  ungroup()

player_df <- read_csv(here("Data/Raw/Player Data.csv"), col_select = -1) |> 
  mutate(utc_start = as_date(utc_start)) |> 
  filter(!(season == season & round > (round_number-2) )) |> # Gets rid of future rounds
  group_by(season, round, player_id) |> 
  slice_head(n = 1) |> # Removes any duplicate records
  ungroup()
...
team_and_recovery_df <- team_df |> 
  ...
  left_join(ladder_df |> bind_rows(current_ladder_df), by = c("team", "season", "round", "comp")) |> 
  select(-team, -season) |> 
...
elo_model <- elo.run(result ~ team_home + team_away, data = elo_input, k = k) |> 
  as_tibble() |> 
...
## -------------------------------------------------------------------------------------------------------------------------------------------------
# Data split for model fitting (training on all historical data, predicting on any matches in data after today)
...
## -------------------------------------------------------------------------------------------------------------------------------------------------
# This chunk makes predictions on upcoming matches, and creates the predictions table, which is imported into Power BI.

set.seed(234)

final_wf <- readRDS(here("Outputs/modelling_pipeline_worklfow.rds"))
...
## -------------------------------------------------------------------------------------------------------------------------------------------------
# This chunk describes the historical odds data, useful for comparing different bookies analytics.

# Collects and cleans new odds

odds_raw <- read_excel(here("Odds-Jacks_L5.xlsx")) |> 
    separate(event_name, into = c("team_1", "team_2"), sep = "_") |> 
    mutate(team_1 = str_extract(team_1, "\\w+$"),
           team_2 = str_extract(team_2, "\\w+$")) |> 
    pivot_longer(
      cols = c(team_1, team_2, odd_1, odd_2),
      names_to = c(".value", "side"),
      names_pattern = "(team|odd)_(\\d+)"
    ) |> 
    select(team, odd, bookmaker, last_update, date = commence) |> 
    mutate(date = as_date(date))

# Joins with prediction data

odds <- predict(final_fit, new_data = pred_data, type = "prob") |> 
  bind_cols(pred_data) |> 
  select(1:4, away_team, home_team) |> 
  mutate(pred = ifelse(.pred_H > .pred_A, home_team, away_team)) |> 
  pivot_longer(
  ...
  mutate(home_away = ifelse(home_away == "away_team", "away", "home")) |> 
  pivot_wider(
    names_from = home_away,
    values_from = c(team, odd),
    names_glue = "{home_away}_{.value}"
  ) |> 
  select(-1:-2, -pred)

odds_log <- read_csv(here("Outputs/odds_log.csv"), col_select = -1) |> 
  mutate(date = as_date(date),
...
write.csv(odds_df, here("Outputs/odds_log.csv"))


## -------------------------------------------------------------------------------------------------------------------------------------------------
# This chunk describes the actual bets being made, and is imported into Power BI

bets <- predict(final_fit, new_data = pred_data, type = "prob") |> 
  bind_cols(pred_data) |> 
  select(1:4, away_team, home_team) |> 
  mutate(pred = ifelse(.pred_H > .pred_A, home_team, away_team)) |> 
  pivot_longer(
  ...
  group_by(match_id) |> 
  mutate(best_home_odds = max(home_odd),
         best_away_odds = max(away_odd),
         home_ev = .pred_H * best_home_odds - 1,
         away_ev = .pred_A * best_away_odds - 1,
         ev_bet = ifelse(home_ev > away_ev, home_team, away_team),
         odd_diff = ifelse(home_ev >= away_ev, best_home_odds - home_odd, best_away_odds - away_odd)) |> 
  arrange(odd_diff) |> 
  slice_head(n = 1) |> 
  select(match_id, date, home_team, away_team, home_odd, away_odd, bookmaker, ev_bet)

bets_log <- read_csv(here("Outputs/bets_log.csv"), col_select = -1) |> 
  mutate(date = as_date(date))
...
write.csv(bets_df, here("Outputs/bets_log.csv"))
